...
* 現在開発中の3.0.0Alpha16は4/4リリース予定です。進捗内容の確認はlink:https://github.com/hengband/hengband/issues?q=milestone%3A%223.0.0Alpha16Release%22[マイルストーン]にてご確認下さい。
...
### (2021/04/01) エイプリルフールネタ完了と共に公式WebをGitHubへ移行

各位への相談の上、馬鹿馬鹿蛮怒の最初期Alphaリリースの公開をさせていただきました。
極端な「例のアレ」ネタは変愚では決してやりませんが、常に変愚からのマージを受けつつ、一部まともな機能拡張をフィードバックする実験場とて運用する予定です。
馬鹿馬鹿蛮怒のリンクは今後はlink:https://sikabane-works.github.io/bakabakaband/[こちら]になります。
